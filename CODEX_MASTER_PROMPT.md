...
Добавлено:
- Alembic-миграция `0014_rbac_seed_bootstrap_fix` для идемпотентного bootstrap RBAC: создание роли `SUPER_ADMIN`, назначение доступов к модулям и базовых permissions, а также автоматическая привязка роли пользователю `admin`.
Изменено:
- Seed-стратегия RBAC усилена под существующие БД: добавлено выравнивание sequence для `auth_roles` в PostgreSQL перед вставками, исключены неидемпотентные вставки в новом bootstrap-слое.
Удалено:
- Нет.
Причина:
- Закрыть e2e-пробелы в доступах модулей `employees`/`admin` и убрать необходимость ручных SQL-действий при развертывании.
Риски/заметки:
- Миграция рассчитывает на стандартный PostgreSQL dialect для sequence-sync; для иных СУБД шаг синхронизации sequence безопасно пропускается.
