...
- backend может отдавать `is_visible=false` для legacy/internal модулей
- frontend НЕ вычисляет доступ и НЕ знает роли
- если нет доступа или `is_visible=false` → модуль НЕ показывается в sidebar
...
- Frontend-модуль `employees` с тремя вкладками: «Пользователи», «Оргструктура», «Роли», интегрированный в platform moduleRegistry.
- Alembic-миграция `0013_employees_module` с созданием схемы, индексов, ограничений и bootstrap данных для модуля.
Изменено:
- Реестр backend-модулей: добавлено подключение router нового модуля.
- Alembic env: добавлен импорт моделей нового модуля для согласованной metadata.
- Frontend module registry: добавлен модуль `employees` как простой key->component mapping без вычислений.
Удалено:
- Нет.
Причина:
- Реализовать end-to-end админ-модуль «Сотрудники» с сохранением модульной архитектуры, backend-first RBAC и инвариантов платформы.
Риски/заметки:
...
### [2026-03-17] — work
Добавлено:
- Контракт Modules API расширен полем `is_visible` (optional, default=true) для скрытия legacy/internal модулей в пользовательском sidebar без удаления backend-модуля.
- В frontend runtime-пропсы модулей добавлен `module`, чтобы UI-модуль мог безопасно брать `module.title` с fallback.
Изменено:
- Модуль «Сотрудники» стабилизирован: безопасная инициализация активной вкладки, fallback на первую вкладку, обработка loading/empty состояний и защита от чтения полей у `undefined/null`.
- Sidebar фильтрует модули по `has_access && is_visible !== false`, устраняя дублирование «Администрирование» и «Сотрудники» в основном пользовательском меню.
- Backend `/modules` начал помечать `admin` как `is_visible=false` (legacy/internal в UI) без изменения RBAC/доступов и без удаления модуля из реестра.
Удалено:
- Нет.
Причина:
- Устранить падение frontend в EmployeesModule и закрепить «Сотрудники» как основной пользовательский модуль управления пользователями, оргструктурой и ролями без поломки существующего backend-кода.
Риски/заметки:
- Прямой переход по URL `/app/modules/admin` остаётся доступным для ролей с правами, но модуль скрыт из sidebar для исключения дублирования пользовательского сценария.
...
- is_visible (bool, optional; default=true)

Инварианты:
- has_access обязан присутствовать всегда.
- permissions обязан присутствовать (может быть {}).
- если is_visible=false, модуль считается legacy/internal и не показывается в sidebar.
