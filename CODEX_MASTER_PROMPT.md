...
Добавлено:
- Alembic-миграция `0014_employees_rbac_fix` для idempotent bootstrap системной роли `SUPER_ADMIN`, автопривязки доступа ко всем platform-модулям и автоматического назначения этой роли пользователю `admin` (если он существует).
- Дополнительные module-level permissions `admin.view` и `employees.view` в bootstrap списка прав модуля «Сотрудники».
Изменено:
- Runtime bootstrap auth-хранилища теперь выполняет безопасный RBAC seed: гарантирует `SUPER_ADMIN`, доступы к модулям, ключевые permissions для `admin`/`employees` и автопривязку к пользователю `admin` без ручного SQL.
Удалено:
- Нет.
Причина:
- Устранить ситуацию, когда `employees` и `admin` скрываются в UI из-за неполного RBAC seed (`has_access=false` и пустые permissions), и убрать необходимость ручного наполнения БД.
Риски/заметки:
- Для полностью существующих окружений без переменной `DATABASE_URL` автоматическая проверка `alembic upgrade head` не выполняется до настройки окружения; сами миграции и bootstrap реализованы idempotent и безопасны к повторному запуску.
