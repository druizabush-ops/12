...
### [2026-03-05] — work
Добавлено:
- UX удаления правил автозадач: endpoint DELETE /counterparties/{id}/auto-tasks/{rule_id}, кнопка удаления в UI с confirm, stop генерации через отвязку recurrence_state у master.
- В интерфейсе автозадач добавлены карточки в стиле Tasks: заголовок, расписание, chips исполнителей, статус-бейджи 🟢/🟡/⚫ и иконки действий.
Изменено:
- Форма контрагента и автозадач переведены на человеко-читаемые дни недели (ПН..ВС), time picker для времени заявки и кнопки очистки времени.
- 4-колоночный layout counterparties визуально разделён заголовками/фоновой дифференциацией/вертикальными границами; выбранные элементы дерева и списка подсвечиваются заметнее.
- Действия edit/pause/resume/stop/delete получили loading-disable, toast-уведомления и обязательный refetch правил после операции.
Причина:
- Довести Autotasks UX до рабочего состояния с предсказуемыми результатами действий и читаемым расписанием.
Риски/заметки:
- Удаление правила реализовано как физическое удаление записи правила (исторические задачи сохраняются), без добавления нового состояния в схему БД.
